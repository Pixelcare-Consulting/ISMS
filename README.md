...
| **Planogram** | Per-branch SKU shelf capacity, MIL, order enforcement; official Excel template (sap_code, sku, max_qty, mil_days) download + import |
...
| `/settings/planning`, `/planogram` | `forecast.manage` / `planogram.*` (Planogram: Download template + upload; SKUs/branches must already exist; BRS forecast CSV stays on Planning) |

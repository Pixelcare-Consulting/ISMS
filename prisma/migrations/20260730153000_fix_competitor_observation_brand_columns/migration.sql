-- Repair: 20260730110000_competitor_brand_models was resolve-marked applied after a failed
-- run (applied_steps_count 0). Lookup tables exist; observation columns were never altered.

-- Ensure lookup tables exist (no-op if already created by partial apply)
...
CREATE INDEX IF NOT EXISTS "competitor_brands_tenant_id_idx" ON "competitor_brands"("tenant_id");
CREATE UNIQUE INDEX IF NOT EXISTS "competitor_brands_tenant_id_name_key" ON "competitor_brands"("tenant_id", "name");
CREATE INDEX IF NOT EXISTS "competitor_models_tenant_id_idx" ON "competitor_models"("tenant_id");
CREATE INDEX IF NOT EXISTS "competitor_models_competitor_brand_id_idx" ON "competitor_models"("competitor_brand_id");
CREATE UNIQUE INDEX IF NOT EXISTS "competitor_models_tenant_id_competitor_brand_id_name_key" ON "competitor_models"("tenant_id", "competitor_brand_id", "name");

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'competitor_brands_tenant_id_fkey'
  ) THEN
    ALTER TABLE "competitor_brands"
      ADD CONSTRAINT "competitor_brands_tenant_id_fkey"
      FOREIGN KEY ("tenant_id") REFERENCES "tenants"("id") ON DELETE CASCADE ON UPDATE CASCADE;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'competitor_models_tenant_id_fkey'
  ) THEN
    ALTER TABLE "competitor_models"
      ADD CONSTRAINT "competitor_models_tenant_id_fkey"
      FOREIGN KEY ("tenant_id") REFERENCES "tenants"("id") ON DELETE CASCADE ON UPDATE CASCADE;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'competitor_models_competitor_brand_id_fkey'
  ) THEN
    ALTER TABLE "competitor_models"
      ADD CONSTRAINT "competitor_models_competitor_brand_id_fkey"
      FOREIGN KEY ("competitor_brand_id") REFERENCES "competitor_brands"("id") ON DELETE CASCADE ON UPDATE CASCADE;
  END IF;
END $$;

-- Add new competitor brand/model columns on observations (idempotent)
ALTER TABLE "competitor_observations" ADD COLUMN IF NOT EXISTS "competitor_brand_id" TEXT;
ALTER TABLE "competitor_observations" ADD COLUMN IF NOT EXISTS "competitor_model_id" TEXT;
ALTER TABLE "competitor_observations" ADD COLUMN IF NOT EXISTS "brand_name" TEXT;
ALTER TABLE "competitor_observations" ADD COLUMN IF NOT EXISTS "model_name" TEXT;

-- Backfill competitor brands from distinct observation brand FKs (only if brand_id still exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'competitor_observations' AND column_name = 'brand_id'
  ) THEN
    ...
    WHERE obs.brand_id = b.id
      ...
      AND obs.competitor_brand_id IS NULL;
    ...
    WHERE obs.id = mapped.observation_id
      AND obs.competitor_model_id IS NULL;
  END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS "competitor_observations_tenant_id_competitor_brand_id_idx"
  ON "competitor_observations"("tenant_id", "competitor_brand_id");

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'competitor_observations_competitor_brand_id_fkey'
  ) THEN
    ALTER TABLE "competitor_observations"
      ADD CONSTRAINT "competitor_observations_competitor_brand_id_fkey"
      FOREIGN KEY ("competitor_brand_id") REFERENCES "competitor_brands"("id") ON DELETE SET NULL ON UPDATE CASCADE;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'competitor_observations_competitor_model_id_fkey'
  ) THEN
    ALTER TABLE "competitor_observations"
      ADD CONSTRAINT "competitor_observations_competitor_model_id_fkey"
      FOREIGN KEY ("competitor_model_id") REFERENCES "competitor_models"("id") ON DELETE SET NULL ON UPDATE CASCADE;
  END IF;
END $$;

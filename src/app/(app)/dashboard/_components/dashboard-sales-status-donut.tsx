...
interface DashboardSalesStatusDonutProps {
  data: KpiStatusCount[];
}
...
function SaleStatusDonutTooltip({
  active,
  payload,
  coordinate,
}: TooltipContentProps) {
...
export function DashboardSalesStatusDonut({
  data,
}: DashboardSalesStatusDonutProps) {
  ...
  return (
    <div className="flex min-h-64 flex-col rounded-xl border bg-card p-4 shadow-sm">
      <div className="flex shrink-0 items-start justify-between gap-2">
        <div>
          <h3 className="font-semibold">Sale status mix</h3>
          <p className="text-sm text-muted-foreground">
            Sold, reserved, official sold, and to-follow lines
          </p>
        </div>
        <Link
          href="/sales"
      ...
      {total === 0 || chartData.length === 0 ? (
        <div className="flex flex-1 items-center justify-center py-8 text-sm text-muted-foreground">
          No sale lines in your branches yet.
        </div>
      ) : (
          ...
          <div className="relative mx-auto h-44 w-full max-w-52 shrink-0">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip
                  content={SaleStatusDonutTooltip}

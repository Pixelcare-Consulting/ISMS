...
import {
  applyDealerImportChunkAction,
  downloadDealerImportTemplateAction,
  previewDealerImportAction,
} from "@/features/dealers/actions/dealer-import.actions";
import type { DealerImportPreview } from "@/features/dealers/schemas/dealer-import.schema";
...
export function ImportDealersDialog({
  ...
  const [preview, setPreview] = useState<DealerImportPreview | null>(null);
  ...
  function handleTemplate() {
    startTransition(async () => {
      const base64 = await downloadDealerImportTemplateAction();
      downloadWorkbook(base64, "dealers-import-template.xlsx");
    });
  }
  ...
  function handleFile(selected: File) {
    ...
    startTransition(async () => {
      const result = await previewDealerImportAction(formData);
      if ("error" in result) {
    ...
    startTransition(async () => {
      ...
      try {
        for (;;) {
          ...
          const progress = await applyDealerImportChunkAction(formData);
          ...
          if (progress.done) {
            toast.success(
              `${created} created · ${updated} updated · ${unchangedCount} unchanged`,
            );
            setApplyProgress(null);
            handleClose(false);
            router.refresh();
  ...
  const showChangesColumn =
    preview?.rows.some((row) => row.action !== "skip" && row.changes.length > 0) ?? false;
      ...
      >
        ...
        <div className="space-y-4">
          <div className="text-muted-foreground space-y-2 text-sm">
            <p>
              Download the <strong>Dealers</strong> template — it lists your current dealers —
              edit or add rows, then upload that same file. Area, dealer type, dealer area and
              mode of payment are matched by name and must already exist in Settings.
            </p>
            <p>
              Rows match an existing dealer by SAP code, or by name when the code is blank.
              Unmatched rows create a dealer. Blank cells leave a field as it is; rows left out
              of the file are not removed.
            </p>
          </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {preview.rows.map((row) => (
                        <TableRow key={`${row.rowNumber}-${row.sapCode ?? row.name}`}>

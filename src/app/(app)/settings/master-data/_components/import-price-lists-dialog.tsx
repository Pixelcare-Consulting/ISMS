...
import {
  applyPriceListImportChunkAction,
  downloadPriceListImportTemplateAction,
  previewPriceListImportAction,
} from "@/features/master-data/actions/price-list-import.actions";
import type { PriceListImportPreview } from "@/features/master-data/schemas/price-list-import.schema";
...
export function ImportPriceListsDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  function handleTemplate() {
    startTransition(async () => {
      const base64 = await downloadPriceListImportTemplateAction();
      downloadWorkbook(base64, "price-lists-import-template.xlsx");
    });
  }
  ...
  function handleFile(selected: File) {
    ...
    startTransition(async () => {
      const result = await previewPriceListImportAction(formData);
      if ("error" in result) {
    ...
    startTransition(async () => {
      ...
      try {
        for (;;) {
          ...
          const progress = await applyPriceListImportChunkAction(formData);
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      ...
      >
        <DialogHeader>
          <DialogTitle>Import price lists</DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <div className="text-muted-foreground space-y-2 text-sm">
            <p>
              Download the <strong>Price lists</strong> template, fill in SKU, amount, and
              period dates (optional package type), then upload that same file. Excel date
              cells are accepted; CSV dates should be YYYY-MM-DD.
            </p>
            <p>
              SKUs and package types must already exist — this import does not create models.
              Matching periods update amount; rows left out of the file are not removed.
            </p>
          </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {preview.rows.map((row) => (
                        <TableRow
                          key={`${row.sku}-${row.packageTypeName ?? "none"}-${row.periodStart}-${row.rowNumber}`}
                        >

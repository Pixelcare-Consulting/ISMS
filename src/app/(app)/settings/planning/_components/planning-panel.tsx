...
export function PlanningPanel({
  ...
  function runAction(label: string, fn: () => Promise<{ error?: string; success?: boolean }>) {
      ...
      {period ? (
        <>
          <section className="space-y-2">
            <h2 className="text-sm font-medium">Branch revenue targets</h2>
        ...
        <p className="text-sm text-muted-foreground">
          No active planning period. Upload the BRS Planogram &amp; Forecast CSV to begin.
        </p>
      )}
    </div>
  );

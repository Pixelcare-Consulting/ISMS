...
export const BRAND_LOGO_MAX_DIMENSION = 128;

export function isAllowedBrandImage(file: File) {
...
export async function processBrandImage(file: File): Promise<string> {
  if (!isAllowedBrandImage(file)) {
  ...
  const scale = Math.min(
    1,
    BRAND_LOGO_MAX_DIMENSION / Math.max(bitmap.width, bitmap.height),
  );
